  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.fornecedor}>Fornecedor: {item.fornecedor || 'Não informado'}</Text>
        <Text>Preço: R$ {precoValido.toFixed(2)} / un</Text>
        <Text>Estoque: {estoque}</Text>

        <View style={styles.quantidadeContainer}>
          <TouchableOpacity onPress={() => atualizarQuantidade(item.id, -1)} style={styles.qtdBtn}>
            <Text>-</Text>
          </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.botaoCarrinho} onPress={() => adicionarAoCarrinho(item.id)}>
