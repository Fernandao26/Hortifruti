...
const VendasScreen = () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged(user => {
      if (user) {
        ...
        Alert.alert("Acesso Negado", "Por favor, faça login como fornecedor para ver suas vendas.");
        setLoading(false);
  ...
  useEffect(() => {
    ...
    const unsubscribeFirestore = onSnapshot(q, async (snapshot) => {
      ...
      snapshot.docs.forEach(docPedido => {
        ...
        if (pedidoData.carrinho && Array.isArray(pedidoData.carrinho)) {
          pedidoData.carrinho.forEach(itemCarrinho => {
            const promise = getDoc(doc(db, 'produtos', itemCarrinho.produtoId || itemCarrinho.id))
              .then(docProduto => {
              ...
              .catch(error => {
                console.error("Erro ao buscar detalhes do produto para item do carrinho:", itemCarrinho.produtoId || itemCarrinho.id, error);
              });
      ...
      aprovadasTemp.sort((a, b) => b.dataPedido.getTime() - a.dataPedido.getTime());
      pendentesTemp.sort((a, b) => b.dataPedido.getTime() - a.dataPedido.getTime());
      
      setVendasAprovadas(aprovadasTemp);
      setVendasPendentes(pendentesTemp);
      setLoading(false);
      console.log("VendasScreen: Vendas carregadas em tempo real para fornecedor:", currentFornecedorId, "Aprovadas:", aprovadasTemp.length, "Pendentes:", pendentesTemp.length);
    }, (error) => {
  ...
  const renderVendaItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.productDetails}>{'Qtde: '}{item.quantidade}</Text>
        <Text style={styles.productDetails}>{'Preço Un.: R$ '}{item.precoUnitario?.toFixed(2).replace('.', ',')}</Text>
        <Text style={styles.productDetails}>{'Total: R$ '}{((item.quantidade || 0) * (item.precoUnitario || 0))?.toFixed(2).replace('.', ',')}</Text>
        <Text style={styles.orderDateTime}> {/* Nova estilização para data e hora */}
          {'Pedido em: '}{item.dataPedido ? item.dataPedido.toLocaleDateString('pt-BR') : 'N/A'} {'às '}{item.dataPedido ? item.dataPedido.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : ''}
        </Text>
        <Text style={[styles.orderStatus, item.statusPedido === 'approved' ? styles.statusApproved : styles.statusPending]}>
          {'Status: '}{item.statusPedido}
        </Text>
        <Text style={styles.customerInfo}>{'Cliente: '}{item.nomeCliente || item.clienteEmail || 'N/A'}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.fullScreen}>
      ...
      <View style={styles.tabsContainer}>
        <TouchableOpacity 
          style={[styles.tabButton, currentTab === 'aprovadas' && styles.tabButtonActive]}
          onPress={() => setCurrentTab('aprovadas')}
        >
          <Text style={[styles.tabText, currentTab === 'aprovadas' && styles.tabTextActive]}>{'Aprovadas'}</Text>
        </TouchableOpacity>
        <TouchableOpacity 
          style={[styles.tabButton, currentTab === 'pendentes' && styles.tabButtonActive]}
          onPress={() => setCurrentTab('pendentes')}
        >
          <Text style={[styles.tabText, currentTab === 'pendentes' && styles.tabTextActive]}>{'Pendentes'}</Text>
        </TouchableOpacity>
      </View>
      ...
      {currentTab === 'aprovadas' ? (
        vendasAprovadas.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{'Nenhuma venda aprovada encontrada para você.'}</Text>
            <Text style={styles.emptySubText}>{'Aguarde novos pedidos aprovados!'}</Text>
          </View>
        ) : (
          <FlatList
        ...
        vendasPendentes.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>{'Nenhum pedido pendente para você.'}</Text>
            <Text style={styles.emptySubText}>{'Aguarde a aprovação de novos pedidos!'}</Text>
          </View>
        ) : (
          <FlatList
...
const styles = StyleSheet.create({
  ...
  orderDateTime: { // Estilo para data e hora
    fontSize: 12,
    color: '#999',
    marginTop: 5,
  },
