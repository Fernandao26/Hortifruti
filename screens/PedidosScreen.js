...
const PedidosScreen = () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged(user => {
      ...
      if (!user) {
        setLoading(false);
        setPedidos([]); 
        Alert.alert("Acesso Negado", "Você precisa estar logado para ver seus pedidos.");
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        setLoading(false);
        console.log("PedidosScreen: Pedidos do usuário carregados em tempo real.");
      }, (error) => {
  ...
  const renderPedidoItem = ({ item }) => {
    const totalItens = item.carrinho ? item.carrinho.reduce((sum, cartItem) => sum + cartItem.quantidade, 0) : 0;
    ...
    return (
      ...
      >
        ...
        <Text style={styles.pedidoIdText}>{'ID do Pedido: #' + item.id.substring(0, 8)}</Text> 
        
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Status:</Text>
          <Text style={[styles.infoValue, getStatusStyle(item.status)]}>{translateStatus(item.status)}</Text> 
        </View>
        <View style={styles.infoRow}>
          <Text style={styles.infoLabel}>Total do Pedido:</Text>
          <Text style={styles.infoValue}>R$ {item.total?.toFixed(2) || '0.00'}</Text>
        </View>
        <View style={styles.infoRow}>
        ...
        <Text style={styles.sectionTitle}>{'Itens do Pedido (' + totalItens + ' total):'}</Text>
          ...
          item.carrinho.map((cartItem, index) => (
            <View key={index} style={styles.productItem}>
              ...
              <Text style={styles.productDetails}>
                {cartItem.quantidade}x - R$ {(cartItem.preco * cartItem.quantidade).toFixed(2)}
              </Text>
            </View>
          ))
        ) : (
